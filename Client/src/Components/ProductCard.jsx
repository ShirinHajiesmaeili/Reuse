   ...
   return (
       <div className='bg-white p-5 rounded-xl shadow-sm transform transition-all duration-500 hover:scale-105 hover:shadow-2xl relative group'>
           <div className='relative z-10'>
               ...
               <h3 className='text-lg py-3 font-medium truncate transition-colors duration-300 group-hover:text-[#184e77]'>{name}</h3>
               
               <div className="flex items-center mb-3">
                   {[...Array(5)].map((_, index) => (
                       <svg 
                           key={index}
                           className={`w-4 h-4 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-[360deg] ${index < Math.round(data.rating?.rate || 0) ? "text-yellow-300" : "text-gray-300"}`}
               ...
               <div className='flex justify-between items-center'>
                   <p className='text-xl font-bold text-gray-800 transition-all duration-300 group-hover:scale-110'>{price.toFixed(2)} €</p>
